...
def run_training():
    ...
    if resume_ckpt:
        print(f"🔄 Resuming from: {os.path.basename(resume_ckpt)}")
    ...
    # 🔴 FIX APPLIED: early_stop_callback removed from this list
    trainer = pl.Trainer(
        accelerator='gpu',
        devices=1,
    ...
    print(f"🚀 Resuming Training (Target: 50 Epochs, No Early Stop)...")
...
# 🔴 MANUAL EPOCH SELECTION (Set this to 41 based on your table)
TARGET_EPOCH = 49

print(f"--- 🏥 Model Diagnosis for {PATIENT_ID} (Epoch {TARGET_EPOCH}) ---")
...
if not os.path.exists(latest_pt):
    raise FileNotFoundError(f"❌ Prediction file missing: {latest_pt}")
if not os.path.exists(latest_target_pt):
    raise FileNotFoundError(f"❌ Target file missing: {latest_target_pt}")
...
if os.path.exists(wav_pred_path):
    try:
        ...
        print("⚠️ Audio files found but could not be read.")
...
f
    ...
    print(f"❌ Files not found for Epoch {epoch_to_view}.")
...
try:
    ...
    sys.exit(f"❌ Error loading WaveGlow: {e}")
...
print(f"✅ Loaded. Processing ALL {total_samples} samples from Test Set.")
